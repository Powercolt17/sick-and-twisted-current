...
// pay first; every distinct paying position (and every bomb-destroyed cell) doubles once for the next board.
...
export function resolveOutcome(out,bet=1,remainingCents=Math.round(bet*100)*MAX_AWARD){
 ...
 for(const step of out.steps){
  ...
  // Trickster: every distinct paying position AND every cell a bomb destroys doubles once for the next board.
  const grown=positions&&!result.maxWin?[...(raw.exactTenths>0?result.cells:[]),...(step.bomb?.cells||[])]:[];
  const next=grown.length?growPositionMultipliers(positions,grown):positions;
