...
const TRICKSTER_GROW_HOLD=800,TRICKSTER_STAGGER=85;   // tumble-ms; several upgrades strike one after another
...
// Bloody rope (Hang 'Em High, All In Spins) and fire + bone crow (Hell To Pay). Lazy: never delays the base game.
...
const BASE_SOUND_KEYS=['vault','ropeBurn','ropeFizzle','spin','scatter','scatterB','click','stop0','stop1','stop2','stop3','stop4','stop5'];
...
// Trickster brand sting, one per upgraded cell, in strike order. Each strike is a hammer on hot iron (inharmonic
// clang), a twanging string note on E-minor pentatonic, a low thud and a long sizzle. The note climbs with the
// multiplier's size AND with every reveal in the same spin, so a chain of doublings rings higher and higher.
const BRAND_NOTES=[164.81,196,220,246.94,293.66,329.63,392,440,493.88,587.33,659.25,783.99,880,987.77,1174.66];
let brandCombo=0;
function brandSound(hits,staggerMs=0){
 if(muted||!hits.length)return;soundInit();if(!audio)return;
 const start=audio.currentTime+.004,base=Math.min(4,brandCombo);brandCombo++;
 hits.forEach(({value,c},i)=>{
  const t=start+i*staggerMs/1000,step=Math.min(BRAND_NOTES.length-1,Math.max(0,Math.round(Math.log2(value))-1)+base+Math.min(3,i>>1)),f=BRAND_NOTES[step];
  const pan=audio.createStereoPanner?audio.createStereoPanner():null,out=audio.createGain();out.gain.value=.9;
  if(pan){pan.pan.value=Math.max(-.7,Math.min(.7,(c-2.5)/3.6));out.connect(pan);pan.connect(bus());}else out.connect(bus());
  // hammer on hot iron: bright inharmonic partials with fast, uneven decays
  for(const [ratio,gain,decay] of [[4,.13,.7],[4*2.76,.065,.42],[4*5.4,.04,.26],[4*8.93,.022,.16],[4*13.3,.012,.1]]){
   const o=audio.createOscillator(),g=audio.createGain();o.type='sine';o.frequency.setValueAtTime(f*ratio,t);
   g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(gain,t+.002);g.gain.exponentialRampToValueAtTime(.0001,t+decay);o.connect(g);g.connect(out);o.start(t);o.stop(t+decay+.02);}
  // twanging string: bright saw through a closing low-pass, a small downward bend like a plucked wire
  {const o=audio.createOscillator(),lp=audio.createBiquadFilter(),g=audio.createGain();o.type='sawtooth';
   o.frequency.setValueAtTime(f*1.035,t);o.frequency.exponentialRampToValueAtTime(f,t+.06);
   lp.type='lowpass';lp.Q.value=6;lp.frequency.setValueAtTime(4200,t);lp.frequency.exponentialRampToValueAtTime(520,t+.5);
   g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(.075,t+.004);g.gain.exponentialRampToValueAtTime(.0001,t+.85);
   o.connect(lp);lp.connect(g);g.connect(out);o.start(t);o.stop(t+.9);}
  // weight and heat
  thump(t,92,30,.2,i?.06:.1,'sine',out);
  burst(t+.015,5400,1.3,.55,.028,out);if(!i)burst(t+.02,1900,.9,.35,.02,out);
 });
 if(hits.length>2){const t=start+hits.length*staggerMs/1000+.02,f=BRAND_NOTES[Math.min(BRAND_NOTES.length-1,base+7)];   // a chain ends on a ringing octave
  for(const [ratio,gain,decay] of [[2,.06,1.1],[4,.035,.8],[4*2.76,.02,.5]]){const o=audio.createOscillator(),g=audio.createGain();o.type='triangle';o.frequency.value=f*ratio;g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(gain,t+.004);g.gain.exponentialRampToValueAtTime(.0001,t+decay);o.connect(g);g.connect(bus());o.start(t);o.stop(t+decay+.02);}}
}
...
async function tumbleTo(next,cells,token,{blast=false}={}){
 ...
 payout.clear({keepAudio:true});winning=[];for(const k in reelDust)delete reelDust[k];const trickSpeed=turbo?1.65:1/1.12,trickAt=performance.now(),trickHits=tricksterGrid.pendingHits(cells),trickStagger=trickHits.length>1?Math.min(TRICKSTER_STAGGER,420/(trickHits.length-1)):0,trickHold=!reduced&&trickHits.length?TRICKSTER_GROW_HOLD+(trickHits.length-1)*trickStagger:0;tumble.begin(reels,next.grid,cells,trickAt,trickSpeed,{wilds,blast,hold:trickHold,anticipation:eligible&&landedScatters>=2&&landedScatters<5,filler:M.SYMBOLS.filter(s=>!['scatter','max',M.BOX_WILD,M.SUBSTITUTE].includes(s)),booster:paidMode()==='boost'});tricksterGrid.reveal(cells,trickAt+(blast?BLAST_TUMBLE_TIME.clear:TUMBLE_TIME.clear)/trickSpeed,(trickHold?TRICKSTER_GROW_HOLD:0)/trickSpeed,trickStagger/trickSpeed);if(trickHits.length)setTimeout(()=>{if(token!==sequenceToken)return;if(!reduced)impactMotion.kick(2.2);brandSound(trickHits.map(h=>({value:h.to,c:h.c})),trickStagger/trickSpeed);},((blast?BLAST_TUMBLE_TIME.clear:TUMBLE_TIME.clear)+(trickHold?TRICKSTER_GROW_HOLD*TRICKSTER_BRAND.hit:0))/trickSpeed);tumble.setPaused(document.hidden,performance.now());
