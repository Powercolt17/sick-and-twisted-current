...
export function createPayoutPresentation({ctx,G,reduced,onSilence=()=>{},makeCanvas,largeWin=null,crossfire=null}){
 ...
 function begin(value,ways,cells,groups,speed=1,now=performance.now(),opts={}){
  ...
  const tier=bigWinTier(value,stake>0?stake:wager,roundReturn),big=!opts.cascade&&tier>0;
 ...
 function drawSummary(now){
  ...
  if(state.value<=0){
   ...
   ctx.save();ctx.globalAlpha=a*ease((t-PAYOUT_TIME.register)/80);ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='bold 16px Western';ctx.fillStyle='#e8dcc3';ctx.fillText(`${feature.reels} SCATTERS · ${feature.name} · ${feature.count} FREE SPINS`,G.x+G.w/2,32);ctx.restore();return;
  }
  if(t<state.amountAt)return;
  if(state.bloodMoney){
   // The bottom timber owns the win receipt; wild multipliers keep their full reel.
   ctx.save();ctx.globalAlpha=a*ease((t-state.amountAt)/60);ctx.fillStyle='#20150fee';ctx.fillRect(G.x+7,G.y+G.h+6,G.w-14,43);
   ctx.textAlign='center';ctx.textBaseline='alphabetic';ctx.lineJoin='round';
   centered('WIN',G.x+G.w*.11,G.y+G.h+28,18,70,'#d5c1a0',1);
   centered(money(state.value),G.x+G.w*.43,G.y+G.h+28,36,G.w*.45,'#fff0d1',1.5);
   centered(`${state.ways.toLocaleString('en-US')} WAYS`,G.x+G.w*.82,G.y+G.h+28,20,G.w*.26,'#e7d7bb',1);ctx.restore();return;
  }
  const u=t-state.amountAt,L=PAYOUT_LAYOUT,cx=G.x+G.w*L.x,cy=G.y+G.h*L.amountY,wy=G.y+G.h*L.waysY;
  ...
  ctx.save();ctx.globalAlpha=a*ease(u/70);ctx.textAlign='center';ctx.textBaseline='alphabetic';ctx.lineJoin='round';
  ctx.translate(cx,cy);ctx.scale(pop,pop);ctx.translate(-cx,-cy);
  // An ink edge and short cast shadow keep ivory readable on dirty parchment.
  ctx.shadowColor='rgba(10,7,5,.75)';ctx.shadowBlur=G.h*.012;ctx.shadowOffsetY=G.h*.006;
  const fill=ctx.createLinearGradient(0,cy-G.h*.08,0,cy+G.h*.08);fill.addColorStop(0,'#fff1d2');fill.addColorStop(.48,'#e7d7b4');fill.addColorStop(1,'#c3ae85');
  const v=state.big&&!reduced&&u<B.count?Math.min(state.value-.01,Math.floor(state.value*ease(u/B.count)*100)/100):state.value;
  centered(amountText(v),cx,cy,G.h*L.amountSize*(state.quietReturn?.65:1),G.w*L.maxWidth,fill,G.h*.007);
  const caption=state.quietReturn?(state.net===0?'RETURN · BREAK EVEN':`RETURN · NET −${money(Math.abs(state.net))} SO FAR`):state.summaryOnly?'ROUND RETURN':`${state.ways.toLocaleString('en-US')} TOTAL WAYS`;
  centered(caption,cx,wy,G.h*(state.quietReturn?.045:L.waysSize),G.w*L.maxWidth,'#eee3cc',G.h*.0045);
  if(state.big)centered('BIG WIN',cx,cy-G.h*.145,G.h*.053,G.w*.6,'#e8dcc3',G.h*.004);
  ctx.restore();
 }
