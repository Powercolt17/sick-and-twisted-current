...
export const FEATURE_MEDIA = Object.freeze({
 blood: {name:'Blood Money',file:'blood-money',scatters:3},
...
export function createFeatureScenes({canvas,stage,isMuted,getView,onMix,onMusic,reduced=false,now=()=>performance.now()}){
 ...
 async function play(key,input={}){
  ...
  skip.textContent=key==='blood'?'OPEN CONTRACT':'CONTINUE';
 ...
 function complete(ok){
  if(!session)return false;const s=session;session=null;controls.hidden=true;resume.hidden=true;s.record.intro.pause();s.record.intro.onended=null;s.record.intro.onerror=null;release(s.record.intro);
 ...
 function setPaused(value){
  ...
  if(value){if(session)session.pausedAt=at;if(exitAt!==null)exitAt={at:exitAt,pause:at};for(const r of media.values()){resources.get(r.intro).pause();resources.get(r.background).pause();}}
  else{
   if(exitAt&&typeof exitAt==='object')exitAt=exitAt.at+at-exitAt.pause;
   const s=session;if(s){if(s.phase==='reveal')s.revealAt+=at-(s.pausedAt??at);s.pausedAt=null;if(s.fallback){s.fallback=false;beginReveal();}else if(s.phase==='film'||s.phase==='waiting'){s.progressAt=at;void playFilm(s);}}
 ...
 function drawIntro(ctx){
  const s=session;if(!s||s.phase==='reveal')return false;
