...
export function infoModel(baseBet=1){
 ...
 const buys=[
  ...
  {id:'deader',title:'Hang ’Em High',art:'assets/shop/no-mercy.webp',cost:M.BUY_COST.deader,description:`Awards ${M.TRIGGER.spins[4]} free spins. The first hanging Outlaw is captured by spin 3. Capture up to three full-reel Wilds, each entering at ×8 and remaining through every spin and tumble until the feature ends. Once three reels are locked, further capture events double one locked multiplier: ×8 → ×16 → ×32 → ×64. The multipliers add together and apply to ordinary wins. There is no additional feature Win Boost. A symbol pays only when its connected winning reels contain at least one regular instance of that symbol; Wild-only symbol groups do not pay. Captures and upgrades can arrive at the start of a spin or after any winning tumble. Three or more visible scatters on the final settled board award +3 spins, once per spin, up to four retriggers (24 spins total). Locked Wilds remain through the extra spins. Regular winners clear; locked Wilds stay. MAX is inactive. Progress resets after the final result.`},
 ...
 return {bet,paytable,modes,buys,rtps,rtp:commonRTP,max:M.MAX_AWARD,reels:M.REELS,rows:M.ROWS,
  free:[
   ...
   {count:4,title:'Hang ’Em High',spins:M.TRIGGER.spins[4],boost:M.MODE_RULES.deaderfree.boost,art:'assets/shop/no-mercy.webp',description:'Four scatters award twelve spins using the same rules as the feature buy. Capture up to three permanent ×8 Outlaw Wild reels; the first arrives by spin 3. After the third capture, further events double a locked multiplier up to ×64. Captures and upgrades can also arrive after winning tumbles. Three or more visible scatters on a settled board add three spins, once per spin, up to four retriggers (24 spins total). Locked reels survive spins and tumbles, including retriggers. Symbol groups need a regular matching symbol to pay. MAX is inactive.'},
