...
export function createHellIntro({canvas,stage,W,H,grid:G,getAudio,isMuted,onMix,onResolution=()=>{},feature={},now=()=>performance.now()}){
 ...
 function draw(ctx){
  ...
  burn.draw(ctx,t);
  if(showScatterCount&&t>=1.48&&t<take.cardAt){ctx.save();ctx.globalAlpha=smooth((t-1.48)/.16);ctx.fillStyle='#22100c';ctx.fillRect(G.x+G.w/2-138,565,276,32);ctx.font='18px HellOutlaw';ctx.fillStyle='#d9c7a0';ctx.textAlign='center';ctx.fillText(`${state.cells.length} SCATTERS`,G.x+G.w/2,587);ctx.restore();}
