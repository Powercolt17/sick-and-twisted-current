...
export function infoModel(baseBet=1){
 ...
 const modes=[
  ...
  {id:'trickster',title:'Trickster Spins',cost:M.SPIN_COST.trickster,boost:M.MODE_RULES.trickster.boost,art:'assets/shop/outlaw-spin.webp',description:`High volatility: most paid spins return nothing. Smaller returns occur alongside occasional larger wins. A payout can be less than the spin cost. Each reel position starts at 1×. A paying square uses its current multiplier, then doubles once for the next tumble: 1× → 2× → 4× → 8× and onward. A square destroyed by a Bomb also doubles once. The multiplier is branded into the board beneath its square: it shows when that square is cleared, and a glowing edge marks multiplied squares while they are covered. It stays in that position as symbols fall or are replaced. Multipliers from the positions in each individual winning way multiply together; for example, 2×, 2× and 2× add an 8× multiplier to that way. Different ways are then added. Other squares do not increase. The existing Win Boost and Outlaw Wild multiplier also apply. All position multipliers reset for each new paid spin and do not carry into awarded free spins. Ordinary Trickster returns before an awarded feature are at most ${number(M.MODE_RULES.trickster.pathCeiling/M.SPIN_COST.trickster)}× the spin cost. MAX and awarded free spins can pay more, up to the overall round limit. MAX can appear and connect.`},
