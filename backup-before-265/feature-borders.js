...
//   All In Spins + Hell To Pay      -> OpenArt fire + bone crow (RGB|matte packed H.264)
...
export const FEATURE_BORDER_ART={
 ...
 inferno:{poster:'assets/feature-borders/inferno-poster.webp',video:'assets/feature-borders/inferno-packed.mp4',size:[864,496],opening:[125,129,599,316]}
};
...
// Source -> world rectangles. Width scales uniformly so the opening's left/right edges land on G;
// header, body and footer are fitted separately so the header stays inside the 608px cabinet.
export function borderSlices([w,h],[x,y,iw,ih],G,worldH=608){
 const sx=G.w/iw,dx=G.x-x*sx,dw=w*sx;
 return [[0,0,w,y,dx,0,dw,G.y],[0,y,w,ih,dx,G.y,dw,G.h],[0,y+ih,w,h-y-ih,dx,G.y+G.h,dw,worldH-G.y-G.h]].filter(s=>s[3]>0&&s[7]>0);
}
const VERTEX='attribute vec2 p;varying vec2 uv;void main(){uv=vec2(p.x*.5+.5,.5-p.y*.5);gl_Position=vec4(p,0.,1.);}';
// Left half = colour, right half = matte (red channel).
const FRAGMENT='precision mediump float;varying vec2 uv;uniform sampler2D art;void main(){vec3 c=texture2D(art,vec2(uv.x*.5,uv.y)).rgb;float a=texture2D(art,vec2(.5+uv.x*.5,uv.y)).r;gl_FragColor=vec4(c,smoothstep(.025,.98,a));}';
export function createFeatureBorders({G,reduced=false,worldH=608,doc=globalThis.document}={}){
 ...
 const ropeRaster=createStaticRaster({width:1212,height:worldH,paint:ctx=>{if(rope)paint(ctx,rope,size(rope),ART.rope.opening);}});
 ...
 function draw(ctx,{cacheStatic=false}={}){
  if(kind==='rope'){if(!rope)return false;if(cacheStatic)ropeRaster.draw(ctx,true);else paint(ctx,rope,size(rope),ART.rope.opening);return true;}
  if(kind==='inferno'){const src=fireSource();if(!src)return false;paint(ctx,src,src===poster?size(poster):ART.inferno.size,ART.inferno.opening);return true;}
  return false;
 }
 async function load(){
  ...
  if(r.status==='fulfilled'){rope=r.value;ropeRaster.clear();}else console.warn('Rope border unavailable; original frame kept.',r.reason);
  if(p.status==='fulfilled')poster=p.value;else console.warn('Fire border poster unavailable; original frame kept.',p.reason);
 ...
 return {load,draw,
  ...
  get state(){return {kind,active:this.active,loaded:{rope:!!rope,poster:!!poster},playing:!video.paused,attached:resource.attached,decoder:!!decoder,time:video.currentTime,failed,reduced,motion};}};
}
