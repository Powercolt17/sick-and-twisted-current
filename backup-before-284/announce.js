...
export function drawAnnouncement(ctx,{x,y,width=606,eyebrow='',lead='',headline='',sub='',scale=1,alpha=1,anchor='center',maxWidth=width*1.15,backing=true}={}){
 if(alpha<=0)return 0;
 ...
 if(lead||headline){ // baseline-aligned lead + headline, shrunk together if too wide
  ...
  if(lead){ctx.font=`700 ${m.ls}px Georgia,serif`;ctx.fillStyle=ANNOUNCE.lead;ctx.fillText(lead,lx,0);lx+=lw+gap;}
