...
export const FEATURE_BORDER_ART={
 rope:{src:'assets/feature-borders/rope.webp',opening:[259,296,1228,832],bands:[131,1261],
  // The crow is cut out of the frame and driven by the base game's crow rig; box = where it sits in rope.webp.
...
export function createFeatureBorders({G,reduced=false,worldH=608,doc=globalThis.document}={}){
 ...
 let ropeCrow=null,crowRig=null;
 const size=im=>[im.naturalWidth||im.width,im.naturalHeight||im.height];
 function paint(ctx,src,dims,opening,bands){
  ctx.save();ctx.beginPath();ctx.rect(-400,-400,G.x+G.w+1200,worldH+400);ctx.rect(G.x,G.y,G.w,G.h);ctx.clip('evenodd');
 ...
 const ropeRaster=createStaticRaster({width:1212,height:worldH,paint:ctx=>{if(rope)paint(ctx,rope,size(rope),ropeOpening,ropeBands);}});
 ...
 function draw(ctx,{cacheStatic=false,time=0,animate=true,compact=false}={}){
  if(kind==='rope'){if(!rope)return false;if(cacheStatic)ropeRaster.draw(ctx,true);else paint(ctx,rope,size(rope),ropeOpening,ropeBands);paintCrow(ctx,time,animate,compact);return true;}
 ...
 async function load(){
  ...
  if(r.status==='fulfilled'){rope=r.value;const m=measureOpening(rope,160);if(m){ropeOpening=m.opening;ropeBands=m.bands;}ropeRaster.clear();}else console.warn('Rope border unavailable; original frame kept.',r.reason);
 ...
 return {load,draw,
  ...
  get state(){return {kind,active:this.active,openings:{rope:ropeOpening,inferno:fireOpening},bands:{rope:ropeBands,inferno:fireBands},loaded:{rope:!!rope,poster:!!poster,crow:!!ropeCrow,rig:!!crowRig},playing:!video.paused,attached:resource.attached,decoder:!!decoder,time:video.currentTime,failed,reduced,motion};}};
}
