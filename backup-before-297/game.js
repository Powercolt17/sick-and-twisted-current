...
import {createTricksterGrid,TRICKSTER_BRAND} from './trickster-grid.js?v=8';
...
const hangPresentation=createHangPresentation({stage:$('#stage'),G,reduced,announce:setStatus,
 // a lantern catching: the supplied fire-lighting recording, from that reel's side, a touch higher for each lantern
 onLantern:({reel,index})=>{if(muted)return;soundInit();if(!audio)return;playSample('lanternIgnite',.85,.94+index*.04,false,0,0,undefined,Math.max(-.6,Math.min(.6,(reel-2.5)/3.6)),1);},
 onImpact:event=>{
...
const featureScenes=createFeatureScenes({canvas,stage:$('#stage'),isMuted:()=>muted,reduced,G,
 sfx:(key,gain=1,rate=1,pan=0)=>{if(muted)return;soundInit();if(!audio)return;playSample(key,gain,rate,false,0,0,undefined,pan,1);},kick:strength=>impactMotion.kick(strength),
...
const BASE_SOUND_KEYS=['vault','ropeBurn','ropeFizzle','spin','scatter','scatterB','click','lanternIgnite','stop0','stop1','stop2','stop3','stop4','stop5'];
...
// Trickster: when a cell's border flames up (the multiplier strikes), it plays the same fire-lighting recording as the
// Hang 'Em High lanterns (assets/sfx/lantern-ignite.wav), one per upgraded cell in strike order, panned to its column.
// It climbs gently with the multiplier (x2 as recorded -> about +35% pitch at x64) and a little with each hit in the same
// tumble, so a chain still rises without the fire turning into a chirp. x32 and up keep the heavy impact underneath.
function brandSound(hits,staggerMs=0){
 if(muted||!hits.length)return;soundInit();if(!audio||!samples.lanternIgnite)return;
 hits.forEach(({value,c},i)=>{
  const tier=Math.max(1,Math.round(Math.log2(value))),rate=Math.min(1.45,Math.pow(2,(tier-1)*.085+i*.03+Math.min(3,brandCombo)*.015));
  playSample('lanternIgnite',.9+.02*Math.min(5,tier),rate,false,i*staggerMs,0,undefined,Math.max(-.7,Math.min(.7,(c-2.5)/3.6)),1);
