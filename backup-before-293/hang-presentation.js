...
export function createHangPresentation({stage,G,reduced=false,announce=()=>{},onImpact=()=>{},onFinale=()=>{},onLantern=()=>{}}){
 ...
 return {
  ...
  draw(ctx,now,{mobile=false,header=0,remaining=0,total=0}={}){
   ...
   else{ // a plank on the rail: the feature title burnt into the wood, counters on iron tags at each end
    const y=G.y+G.h+42,x0=G.x-7,x1=G.x+G.w+7,h=29,top=y-11;
    const wood=ctx.createLinearGradient(0,top,0,top+h);wood.addColorStop(0,finale?'#5a2a16':'#4a3220');wood.addColorStop(.5,finale?'#3c1a0e':'#2c1d12');wood.addColorStop(1,finale?'#2a1008':'#1c120b');
    ctx.fillStyle=wood;ctx.fillRect(x0,top,x1-x0,h);ctx.strokeStyle='#120a05';ctx.lineWidth=1.5;ctx.strokeRect(x0+.5,top+.5,x1-x0-1,h-1);ctx.fillStyle='rgba(255,200,140,.10)';ctx.fillRect(x0+2,top+2,x1-x0-4,1);
    const tag=(tx,align,label,color)=>{ctx.font='700 11px Arial,sans-serif';let w=0;for(const ch of label)w+=ctx.measureText(ch).width+2.4;w+=16;const bx=align==='left'?tx:tx-w;
     ctx.fillStyle='#262220';ctx.fillRect(bx,top+5,w,h-10);ctx.strokeStyle='#7a6650';ctx.lineWidth=1;ctx.strokeRect(bx+.5,top+5.5,w-1,h-11);ctx.fillStyle='#9a9088';for(const rx of [bx+4,bx+w-4]){ctx.beginPath();ctx.arc(rx,y+3,1.6,0,Math.PI*2);ctx.fill();}
     ctx.fillStyle=color;ctx.textAlign='left';ctx.textBaseline='middle';let cx=bx+8;for(const ch of label){ctx.fillText(ch,cx,y+3);cx+=ctx.measureText(ch).width+2.4;}};
    tag(x0+8,'left',finale?'LAST SENTENCE · DOUBLE STAMPS':`${order.length}/3 OUTLAWS · 3 STAMPS TO DOUBLE`,'#d5ad6e');tag(x1-8,'right',`${remaining} SPINS LEFT`,'#e9d7ad');
    // the brand: dark burnt letters with a faint ember edge
    const title=(finale?'THE LAST SENTENCE':'HANG ’EM HIGH')+(combined?`   ·   ×${combined}`:'');ctx.font='700 17px Outlaw,Western,Georgia,serif';ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.save();ctx.globalCompositeOperation='lighter';ctx.shadowColor='rgba(255,140,50,.55)';ctx.shadowBlur=9;ctx.fillStyle='rgba(255,120,40,.18)';ctx.fillText(title,G.x+G.w/2,y+3,G.w*.46);ctx.restore();
    ctx.lineJoin='round';ctx.lineWidth=3;ctx.strokeStyle='#3a1a0a';ctx.strokeText(title,G.x+G.w/2,y+3,G.w*.46);ctx.fillStyle='#160904';ctx.fillText(title,G.x+G.w/2,y+3,G.w*.46);ctx.fillStyle=`rgba(255,${finale?110:150},60,.55)`;ctx.fillText(title,G.x+G.w/2,y+2.2,G.w*.46);}
   ctx.restore();
  },
