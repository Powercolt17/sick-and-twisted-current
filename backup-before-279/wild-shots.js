...
export function createWildShots({G,figureEffect,reduced=false,onCue=()=>{},onConvert=()=>{},makeCanvas=()=>document.createElement('canvas')}){
 ...
 function drawBoard(ctx,now,drawSprite){
  ...
  if(run.mark){
   ...
   ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='30px Outlaw,Georgia,serif';ctx.lineJoin='round';
   ctx.strokeStyle='#160e08';ctx.lineWidth=5;ctx.fillStyle='#f4ddb0';
   const caption='OUTLAW’S MARK · '+run.targets.length+' WILDS',cx=G.x+G.w/2,cy=G.y+G.h+27;
   ctx.strokeText(caption,cx,cy,G.w-44);ctx.fillText(caption,cx,cy,G.w-44);ctx.restore();
