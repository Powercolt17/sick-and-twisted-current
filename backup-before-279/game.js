...
import {createTricksterGrid,TRICKSTER_BRAND} from './trickster-grid.js?v=6';
...
async function tumbleTo(next,cells,token,{blast=false}={}){
 ...
 payout.clear({keepAudio:true});winning=[];for(const k in reelDust)delete reelDust[k];const trickSpeed=turbo?1.65:1/1.12,trickAt=performance.now(),trickHold=!reduced&&tricksterGrid.pendingIn(cells)?TRICKSTER_GROW_HOLD:0;tumble.begin(reels,next.grid,cells,trickAt,trickSpeed,{wilds,blast,hold:trickHold,anticipation:eligible&&landedScatters>=2&&landedScatters<5,filler:M.SYMBOLS.filter(s=>!['scatter','max',M.BOX_WILD,M.SUBSTITUTE].includes(s)),booster:paidMode()==='boost'});tricksterGrid.reveal(cells,trickAt+(blast?BLAST_TUMBLE_TIME.clear:TUMBLE_TIME.clear)/trickSpeed,trickHold/trickSpeed);if(trickHold)setTimeout(()=>{if(token!==sequenceToken)return;if(!reduced)impactMotion.kick(2.2);sfx('brand');},((blast?BLAST_TUMBLE_TIME.clear:TUMBLE_TIME.clear)+trickHold*TRICKSTER_BRAND.hit)/trickSpeed);tumble.setPaused(document.hidden,performance.now());
