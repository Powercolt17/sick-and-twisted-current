...
export function createTricksterGrid({G,reduced=false}){
 ...
 function reveal(cells,at,ms){for(const [c,r] of cells){const p=pending.get(key(c,r));if(p&&p.at===null){p.at=at;p.ms=Math.max(0,ms);}}}
 ...
 return {reset,set,animate,finish,cancel,draw,drawBackground,drawEffects,drawMarkers,setPlates,pendingIn,reveal,snapshot,get active(){return !!values},get owner(){return owner}};
}
