...
const model=M.modelEntries('allin'),cost=M.SPIN_COST.allin;
const near=(a,b,label,tol=1e-10)=>assert.ok(Math.abs(a-b)<tol,label+': '+a+' vs '+b);
near(M.targetRTP('allin'),.962,'unchanged target');
assert.equal(cost,2000);assert.equal(M.MAX_AWARD,77777);
...
for(const row of model.rows){
 ...
 for(let i=0;i<row.ids.length;i++){
  ...
  const id=row.ids[i],out=M.outcomeById(id,'allin'),resolved=M.resolveOutcome(out);
  ...
  assert.ok(resolved.value<M.MAX_AWARD);
 ...
 for(const i of [0,Math.floor(row.ids.length/2),row.ids.length-1]){
  ...
  const out=M.rollOutcome('allin',()=>draws[index++]??.999999);
  assert.equal(out.pathId,row.ids[i]);
 }
}
for(const id of model.maxIds){
 const out=M.outcomeById(id,'allin'),resolved=M.resolveOutcome(out);
...
near(ev/cost,M.theoreticalRTP('allin'),'advertised and evaluated RTP');
assert.ok(dead>.85&&dead<.86,'85–86% terminal dead spins');
assert.ok(1-dead>.14&&1-dead<.15,'14–15% hit/feature rate');
near(feature,(.006+.001+.00005)*(1-model.maxProbability),'feature odds preserved');
near(model.maxProbability,1/250000,'MAX odds preserved');
const tail=M.breakdown('allin').bands.find(r=>r.name==='tail');
assert.ok(tail.probability>.01&&tail.probability<.02);
const big=M.breakdown('allin').bands.find(r=>r.name==='big');
assert.ok((tail.mean*tail.probability+big.mean*big.probability)/ev>.65,'large wins carry most return');
...
let misses=0,streak=0,longest=0,marks=0;
for(let n=0;n<50000;n++){
 const out=M.rollOutcome('allin',rng),value=payouts.get(out.pathId);
 assert.notEqual(value,undefined);if(out.outlawsMark)marks++;
 if(!value){misses++;longest=Math.max(longest,++streak);}else streak=0;
 if(n<1000){
  const actual=M.resolveOutcome(out),original=M.resolveOutcome(M.outcomeById(out.pathId,'allin'));
...
assert.ok(marks>1500&&marks<2300,'live sampler emits marked outcomes at the expected incidence');
const report={rtp:ev/cost,deadSpinProbability:dead,hitOrFeatureProbability:1-dead,
 ...
 sample:{spins:50000,deadSpins:misses,longestDeadStreak:longest,markedSpins:marks}};
mkdirSync(new URL('../.sites-runtime/',import.meta.url),{recursive:true});
writeFileSync(new URL('../.sites-runtime/allin-audit.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
