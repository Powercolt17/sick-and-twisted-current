...
export function createPayoutPresentation({ctx,G,reduced,onSilence=()=>{},makeCanvas,largeWin=null,crossfire=null}){
 ...
 function begin(value,ways,cells,groups,speed=1,now=performance.now(),opts={}){
  ...
  state={value,roundReturn,quietReturn,wager,shotSequence,presentationOffset,largeStarted:false,reactionMs,cascade:!!opts.cascade,summaryOnly:!!opts.summaryOnly,ways,keys,paying,reacts,wildReels,net,stake,profitable,big,tier,profile,bonus:!!opts.bonus,groups:groups||[],
 ...
 function drawSummary(now){
  ...
  const B=PAYOUT_TIME.big;
  // A worthwhile win counts up briefly, then lands with a pop and a gold flash; small returns just appear.
  const countMs=!reduced&&!state.big&&state.profitable&&state.value>=3*(state.wager||1)?380:0,landed=u-countMs;
  const pop=reduced||!state.profitable?1:state.big?1+.10*(1-clamp(u/B.rise))**3:countMs?(landed<0?1:1+.09*Math.exp(-landed/80)*Math.cos(landed/60)):1+.055*Math.exp(-u/55)*Math.sin(u/55);
  const glow=countMs&&landed>=0?Math.max(0,1-landed/420):0;
  const v=state.big&&!reduced&&u<B.count?Math.min(state.value-.01,Math.floor(state.value*ease(u/B.count)*100)/100):countMs&&u<countMs?Math.floor(state.value*(1-Math.pow(1-u/countMs,3))*100)/100:state.value;
  ...
  drawAnnouncement(ctx,{x:cx,y:G.y+G.h*((L.amountY+L.waysY)/2),width:G.w*(quiet?.66:.92),eyebrow,lead:amountText(v),sub,scale:pop,alpha:a*ease(u/70),maxWidth:G.w*L.maxWidth,glow});
 }
