...
export function createBloodWallIntro({G,W=1212,H=608,reduced=false,sfx=()=>{},kick=()=>{},doc=globalThis.document}={}){
 const art={};for(const k of ['wall','hand','stamp','contract','nail']){const i=new Image();i.decoding='async';i.onload=()=>{art[k]=i;};i.src=`assets/bounty-wall/${k}.webp`;}
 let state=null;
 function begin(cells,frozen,awardedSpins,now){
  const order=[...cells].sort((a,b)=>a[0]-b[0]||a[1]-b[1]);
  state={cells:order,frozen,spins:awardedSpins||8,start:now,pausedAt:null,fired:new Set(),spatter:null};return state;
 }
 ...
 function posterTargets(F,n){const pw=Math.min(212,F.w*.19),ph=pw*G.ch/G.cw,cx=[.26,.5,.74].map(f=>F.x+F.w*f),cy=F.y+F.h*.40;return Array.from({length:n},(_,i)=>({x:cx[Math.min(i,2)]-pw/2,y:cy-ph/2,w:pw,h:ph,rot:(i-1)*.045+(hash(i)-.5)*.03}));}
 function draw(ctx,now,view){
  ...
  ctx.drawImage(state.frozen,0,0,W,H);
  ...
  const fw=state.frozen.width/W,fh=state.frozen.height/H,targets=posterTargets(F,state.cells.length);
  state.cells.forEach(([c,r],i)=>{
   ...
   ctx.drawImage(state.frozen,sx*fw,sy*fh,G.cw*fw,G.ch*fh,-w/2,-h/2,w,h);ctx.shadowColor='transparent';
  ...
  // 3. the bloody hand
  if(t>=B.hand){const p=clamp((t-B.hand)/140),s=1.5-.5*out(p),hw=F.w*.22,hh=art.hand?hw*art.hand.naturalHeight/art.hand.naturalWidth:hw*1.2,hx=F.x+F.w*.36,hy=F.y+F.h*.34;
  ...
  if(t>=B.stamp){const p=clamp((t-B.stamp)/160),s=2.2-1.2*out(p),sw=F.w*.62,sh=art.stamp?sw*art.stamp.naturalHeight/art.stamp.naturalWidth:sw*.36,sx=F.x+F.w*.5,sy=F.y+F.h*.42;
  ...
  if(t>=B.contract){const p=clamp((t-B.contract)/380),cw=Math.min(300,F.w*.28),ch=art.contract?cw*art.contract.naturalHeight/art.contract.naturalWidth:cw*1.3,cx=F.x+F.w*.5,cy=F.y+F.h*.78+(1-back(p))*F.h*.6;
 ...
 return {begin,draw,elapsed,pause,resume,end,get active(){return !!state;},get state(){return state;}};
}
