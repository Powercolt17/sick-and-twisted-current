...
export function createFeatureScenes({canvas,stage,isMuted,getView,onMix,onMusic,reduced=false,now=()=>performance.now(),sfx=()=>{},kick=()=>{},G={x:267,y:157,w:660,h:396,cw:110,ch:99}}){
 // Blood Money opens on the reels themselves (The Bounty Wall) instead of a film.
 const wall=createBloodWallIntro({G,reduced,sfx,kick});
 ...
 async function play(key,input={}){
  ...
  skip.textContent=key==='blood'?'TEAR IT DOWN':'CONTINUE';
  ...
  if(key==='blood'){ // The Bounty Wall: drawn on the canvas from the confirmed board; no intro film to load
   wall.begin(s.cells,frozen,s.awardedSpins,now());s.phase='film';presentation('film');skip.hidden=false;skip.disabled=false;
