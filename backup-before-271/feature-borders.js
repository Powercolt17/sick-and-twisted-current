...
//   Hell To Pay, All In Spins, Trickster Spins -> OpenArt hell frame, keyed from green (RGB|matte packed H.264)
//   Base, Bounty Booster and Blood Money keep the original reel-frame.js border.
...
export function featureBorderKind({mode='normal',bonus=false,label='',feature=null}={}){
 ...
 // Trickster Spins and All In Spins share the fire border; Bounty Booster and base spins keep the normal one.
 return mode==='allin'||mode==='trickster'?'inferno':'normal';
}
...
export function createFeatureBorders({G,reduced=false,worldH=608,doc=globalThis.document}={}){
 ...
 let ropeCrow=null,crowRig=null,ropeDrips=createBloodDrips(ART.rope.drips||[]);
 ...
 function draw(ctx,{cacheStatic=false,time=0,animate=true,compact=false}={}){
  if(kind==='rope'){if(!rope)return false;if(cacheStatic)ropeRaster.draw(ctx,true);else paint(ctx,rope,size(rope),ropeOpening,ropeBands,ART.rope);paintDrips(ctx,time,animate&&!reduced);paintCrow(ctx,time,animate,compact);return true;}
